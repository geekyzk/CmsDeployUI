  ...
  <div class="app-container">
    ...
    <el-row>
      <div class="chart-container">
        <el-table ref="SystemBlackUserTable" v-loading="loading" :data="data" border fit highlight-current-row style="width: 100%;" @sort-change="sortSignTime" @row-dblclick="openDetails">
          <!-- 操作 -->
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="48%" top="7vh">
      <el-form ref="systemBlackUserForm" class="form-add" :model="systemBlackUserTemp" :rules="rules" label-width="100px" style="width: 100%; padding-left:10px;padding-right:10px">
        <el-form-item label="状态" prop="state">
          <el-switch
            v-model="systemBlackUserTemp.state"
            active-color="#13ce66"
...
<script>
import { listSystemBlackUser, createSystemBlackUser, updateSystemBlackUser, deleteSystemBlackUser } from '@/api/SystemBlackUserApi'
import waves from '@/directive/waves'
import { formatDate } from '@/utils'

export default {
  name: 'SystemBlackUser',
  directives: {
    waves
  },
  data() {
    return {
      ...
      rules: {
        state: [{ required: true, message: '状态为必须参数', trigger: 'change' }]
      }
    }
  },
  created() {
  ...
  methods: {
    list() {
      ...
      this.loading = true
      listSystemBlackUser(params).then(response => {
        this.loading = false
    ...
    resetsystemBlackUserTemp() {
      this.systemBlackUserTemp = {
        state: undefined,
        id: undefined
      }
    },
    handleCreate() {
      this.resetsystemBlackUserTemp()
      this.dialogStatus = 'create'
      this.dialogFormVisible = true
      this.$nextTick(() => {
        this.$refs['systemBlackUserForm'].clearValidate()
      })
    },
    createData() {
      this.$refs['systemBlackUserForm'].validate((valid) => {
        if (valid) {
          this.buttonLoading = true
          createSystemBlackUser(this.systemBlackUserTemp).then(response => {
    ...
    handleUpdate(row) {
      Object.assign(this.systemBlackUserTemp, row)
    ...
    updateData() {
      this.$refs['systemBlackUserForm'].validate((valid) => {
        if (valid) {
          this.buttonLoading = true
          updateSystemBlackUser(this.systemBlackUserTemp.id, this.systemBlackUserTemp).then(response => {
            this.buttonLoading = false
            if (response.data.code === -1) {
              this.$message({
                title: '失败',
    ...
    deleteData(id) {
      this.$confirm('确认删除该系统登陆黑名单, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteSystemBlackUser(id).then(response => {
          if (response.data.code === -1) {
            this.$message({
              title: '失败',
              message: response.data.message,
